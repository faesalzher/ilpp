...
class PpidInformasi extends AUTH_Controller
{
    ...
    {
        if ($this->userdata->level == "3" || $this->userdata->level == "7") {
            $data['userdata'] 	= $this->userdata;
            $data['page'] 		= "Laporan Bidang PPID";
            $data['judul'] 		= "Bidang PPID Permintaan Informasi";
            $data['deskripsi'] 	= "Input Laporan";

            $data['modal_tambah_informasi'] = show_my_modal('modals/modal_tambah_informasi', 'tambah-informasi', $data);

            $this->template->views('ppidInformasi/home', $data);
        } else {
    ...
    public function tampil()
    {
        if ($this->userdata->level == "3" || $this->userdata->level == "7") {
            ...
            $data['dataInformasi']=$this->M_informasi->select_by_month($vbulan, $vtahun);
            $this->load->view('ppidInformasi/list_data', $data);
        } else {
            redirect('error_akses');
        }
    }
    ...
    public function prosesTambah()
    {
        if ($this->userdata->level == "3" || $this->userdata->level == "7") {
            ...
            if ($this->form_validation->run() == true) {
                $result = $this->M_informasi->insert($data);
    ...
    public function update()
    {
        if ($this->userdata->level == "3" || $this->userdata->level == "7") {
            $data['userdata'] 	= $this->userdata;
            $id 				= trim($_POST['id']);
            $data['dataInformasi'] 	= $this->M_informasi->select_by_id($id);

            echo show_my_modal('modals/modal_update_informasi', 'update-informasi', $data);
        } else {
            redirect('error_akses');
        }
    }
    ...
    public function prosesUpdate()
    {
        if ($this->userdata->level == "3" || $this->userdata->level == "7") {
            ...
            $data 	= $this->input->post();
            if ($this->form_validation->run() == true) {
                $result = $this->M_informasi->update($data);
    ...
    public function delete()
    {
        if ($this->userdata->level == "3" || $this->userdata->level == "7") {
            $id = $_POST['id'];
            $result = $this->M_informasi->delete($id);
        } else {
            redirect('error_akses');
        }
    }
    ...
    public function export()
    {
        if ($this->userdata->level == "3" || $this->userdata->level == "7") {
          ...
          $data= mysqli_query($connection, "SELECT * FROM laporan_informasi  WHERE YEAR(tanggal) = '".$vtahun."' AND MONTH(tanggal) = '".$vbulan."'  ");
          //loop the dataTamu
          $contentStartRow = 30;
          ...
          $filename = "Laporan Pelayanan Publik PPID Permintaan Informasi ".$vbulan."-".$vtahun.".xlsx";

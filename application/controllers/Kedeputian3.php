...
class Kedeputian3 extends AUTH_Controller
{
    ...
    public function index()
    {
        if ($this->userdata->level == "7") {
            ...
            $data['judul'] 		= "Bidang Pelayanan Fungsional Kedeputian III";
            $data['deskripsi'] 	= "Input Laporan";

            $data['modal_tambah_kedeputian3'] = show_my_modal('modals/modal_tambah_kedeputian3', 'tambah-kedeputian3', $data);

            $this->template->views('kedeputian3/home', $data);
        } else {
    ...
    public function tampil()
    {
        if ($this->userdata->level == "7") {
            ...
            $deputi = 4;
            $data['dataKedeputian3']=$this->M_kedeputian->select_by_month($vbulan, $vtahun, $deputi);
            $this->load->view('kedeputian3/list_data', $data);
        } else {
            redirect('error_akses');
        }
    }
    ...
    public function update()
    {
        if ($this->userdata->level == "7") {
            $data['userdata'] 	= $this->userdata;
            $id 				= trim($_POST['id']);
            $data['dataKedeputian3'] 	= $this->M_kedeputian->select_by_id($id);

            echo show_my_modal('modals/modal_update_kedeputian3', 'update-kedeputian3', $data);
        } else {
            redirect('error_akses');
        }
    }
    ...
    public function export()
    {
        if ($this->userdata->level == "7") {
          ...
          $data=  mysqli_query($connection, "SELECT * FROM laporan_kedeputian  WHERE YEAR(tanggal) = '".$vtahun."' AND MONTH(tanggal) = '".$vbulan."' AND id_pengguna = '4' ");
          //loop the dataTamu
          ...
          $filename = "Laporan Pelayanan Publik Kedeputian III ".$vbulan."-".$vtahun.".xlsx";

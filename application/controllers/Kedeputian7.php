...
class Kedeputian7 extends AUTH_Controller
{
    ...
    public function index()
    {
        if ($this->userdata->level == "7") {
            ...
            $data['judul'] 		= "Bidang Pelayanan Fungsional Kedeputian VII";
            $data['deskripsi'] 	= "Input Laporan";

            $data['modal_tambah_kedeputian7'] = show_my_modal('modals/modal_tambah_kedeputian7', 'tambah-kedeputian7', $data);

            $this->template->views('kedeputian7/home', $data);
        } else {
    ...
    public function tampil()
    {
        if ($this->userdata->level == "7") {
            ...
            $deputi = 8;
            $data['dataKedeputian7']=$this->M_kedeputian->select_by_month($vbulan, $vtahun, $deputi);
            $this->load->view('kedeputian7/list_data', $data);
        } else {
            redirect('error_akses');
        }
    }
    ...
    public function update()
    {
        if ($this->userdata->level == "7") {
            $data['userdata'] 	= $this->userdata;
            $id 				= trim($_POST['id']);
            $data['dataKedeputian7'] 	= $this->M_kedeputian->select_by_id($id);

            echo show_my_modal('modals/modal_update_kedeputian7', 'update-kedeputian7', $data);
        } else {
            redirect('error_akses');
        }
    }
    ...
    public function export()
    {
        if ($this->userdata->level == "7") {
          ...
          $data=  mysqli_query($connection, "SELECT * FROM laporan_kedeputian  WHERE YEAR(tanggal) = '".$vtahun."' AND MONTH(tanggal) = '".$vbulan."' AND id_pengguna = '8' ");
          //loop the dataTamu
          ...
          $filename = "Laporan Pelayanan Publik Kedeputian VII ".$vbulan."-".$vtahun.".xlsx";

...
class Kedeputian6 extends AUTH_Controller
{
    ...
    public function index()
    {
        if ($this->userdata->level == "7") {
            ...
            $data['judul'] 		= "Bidang Pelayanan Fungsional Kedeputian VI";
            $data['deskripsi'] 	= "Input Laporan";

            $data['modal_tambah_kedeputian6'] = show_my_modal('modals/modal_tambah_kedeputian6', 'tambah-kedeputian6', $data);

            $this->template->views('kedeputian6/home', $data);
        } else {
    ...
    public function tampil()
    {
        if ($this->userdata->level == "7") {
            ...
            $deputi = 7;
            $data['dataKedeputian6']=$this->M_kedeputian->select_by_month($vbulan, $vtahun, $deputi);
            $this->load->view('kedeputian6/list_data', $data);
        } else {
            redirect('error_akses');
        }
    }
    ...
    public function update()
    {
        if ($this->userdata->level == "7") {
            $data['userdata'] 	= $this->userdata;
            $id 				= trim($_POST['id']);
            $data['dataKedeputian6'] 	= $this->M_kedeputian->select_by_id($id);

            echo show_my_modal('modals/modal_update_kedeputian6', 'update-kedeputian6', $data);
        } else {
            redirect('error_akses');
        }
    }
    ...
    public function export()
    {
        if ($this->userdata->level == "7") {
          ...
          $data=  mysqli_query($connection, "SELECT * FROM laporan_kedeputian  WHERE YEAR(tanggal) = '".$vtahun."' AND MONTH(tanggal) = '".$vbulan."' AND id_pengguna = '7' ");
          //loop the dataTamu
          ...
          $filename = "Laporan Pelayanan Publik Kedeputian VI ".$vbulan."-".$vtahun.".xlsx";
